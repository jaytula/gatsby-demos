...
1. Install npm packages `gatsby-plugin-react-helmet` and `react-helmet`
2. In `gatsby-config.js` ensure `siteMetadata.description` and `siteMetadata.author` are set.  Also add `gatsby-plugin-react-helmet` to `plugins` array
3. Create file `src/components/seo.js` as follows:
...
4. Import `SEO` component and use it in `blog-post.js`.  Component can be first child
in `Layout`.  Use `post.frontmatter.title` and `post.excerpt` for `title` and `description` respectively.  Don't forget to add `excerpt` to the graphql query.

5. Try running Lighthouse again to see score!